...
class Centraldofrete_Shipping_Model_Carrier_Centraldofrete extends Mage_Shipping_Model_Carrier_Abstract implements Mage_Shipping_Model_Carrier_Interface
{
    ...
    protected function getQuotes($packages, $freeShipping = false)
    {
        // prepare data
        $data = array(
            'postcode_from' => $this->fromZip,
            'postcode_to' => $this->toZip,
            'total_value' => $this->packageValue,
            'packages' => array(),
        );
        ...
        // send request to API
        $quoteRequest = $this->processServerRequest($data, 'quote');
        ...
        $i = 0;
        foreach ($quoteRequest as $quoteResult) {
            if ($freeShipping == 1 && $i == 0) {
                $quoteCost = 0;
            } else {
                $quoteCost = $quoteResult->value;
            }
            $shippingMethod = $quoteResult->id;
            $shippingTitle = $quoteResult->title;
            $shippingCost = $quoteCost;
            $shippingDelivery = $quoteResult->estimate;
     ...
     * 
     * @todo this method must be adapted to the final API version
     * @todo the final URL must be informed on /Centraldofrete_Shipping/etc/config.xml, record <server_url>
     */
    public function processServerRequest($data, $action)
    {
        $helper = Mage::helper('Centraldofrete_Shipping');
        $url = $helper->getConfigValue('server_url') . $action;
        $username = $helper->getConfigValue('carrier_username');
        $password = $helper->getConfigValue('carrier_password');
