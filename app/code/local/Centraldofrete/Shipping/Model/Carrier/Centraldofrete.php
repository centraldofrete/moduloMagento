...
class Centraldofrete_Shipping_Model_Carrier_Centraldofrete extends Mage_Shipping_Model_Carrier_Abstract implements Mage_Shipping_Model_Carrier_Interface
{
    ...
    protected function validateDimensions($items, $validate = 0)
    {
        ...
        $packages = array();
        $firstPackage = Mage::getModel('Centraldofrete_Shipping/package');
                ...
                foreach ($packages as $pa) {
                    ...
                    if (($pa->getLength() + $itemLength) <= $maxLength && ($pa->getWidth() + $itemWidth) <= $maxWidth && ($pa->getHeight() + $itemHeight) <= $maxHeight && (($pa->getValue() + $itemValue) <= $maxValue) && ($pa->getWeight() + $itemWeight) <= $maxWeight) {
                        ...
                        if ($loop < $packagesCount) {
                            ...
                            $pb = Mage::getModel('Centraldofrete_Shipping/package');
    ...
    protected function getQuotes($packages, $freeShipping = false)
    {
        ...
        $data = array(
            ...
            'tipo_carga' => (int) $this->getConfigData('freight_type'),
            'volumes' => array(),
        );
        ...
        foreach ($packages as $package) {
            ...
            array_push($data['volumes'], $quoteData);
        }
        ...
        // process results
        if (!$quoteRequest || !array_key_exists('cotacoes', $quoteRequest)) {
            return false;
        }
        
        // sort by value, from cheapest to more expensive
        usort($quoteRequest['cotacoes'], function($a, $b) {
    ...
    {
        // prepare data
        $url = Mage::helper('Centraldofrete_Shipping')->getAccessUrl() . $action;
        $accessToken =$this->getServerAccessToken();
        $methodUserToken = Mage::helper('Centraldofrete_Shipping')->getConfigValue('carrier_user_token');

        if (!$url || !$accessToken || !$methodUserToken) {
            Mage::log(__CLASS__ . ': Required parameters to connect to Central do Frete API are missing.');
            return false;
        }
        ...
        curl_setopt($ch, CURLOPT_HTTPHEADER, array('Authorization: Bearer ' . $accessToken, 'Content-Type: application/json', 'Cdf-User-Id: ' . $methodUserToken, 'Content-Length: ' . strlen($data)) );
     ...
     *
     * Get access token on Central do Frete API.
     * @return bool|string
     * @see Central do Frete API Documentation http://api.centraldofrete.com/docs/integracao/primeira-requisicao
     */
	protected function getServerAccessToken()
    {
        // prepare data
        $data['grant_type'] = 'client_credentials';
        $data['client_id'] = $this->getConfigData('carrier_username');
        $data['client_secret'] = $this->getConfigData('carrier_password');
        $data = json_encode($data);
        $url = Mage::helper('Centraldofrete_Shipping')->getAccessUrl() . 'auth';
        ...
        if (!$responseJson) {
            Mage::log(__CLASS__ . ': No response when contacting Central do Frete server. The operation was terminated.');
            return false;
        }

        $response = json_decode($responseJson, true);
        if (array_key_exists('status', $response)) {
            Mage::log(__CLASS__ . ': There was an error when dealing with Central do Frete API. Status code: ' . $response['status'] . '. Error message: ' . $response['detail']);
            return false;
        }

        if (!array_key_exists('access_token', $response)) {
            Mage::log(__CLASS__ . ': There was an error when dealing Central do Frete server. Unknown error.');
            return false;
        }
